...
This guide is for DataSHIELD package developers who want to integrate the dsHPC system into their packages. It provides information on how to use dsHPC to submit computationally intensive tasks to Slurm HPC resources, while maintaining compatibility with the DataSHIELD framework.
...
- Running R functions on Slurm-managed nodes
...
The system operates in two modes based on Slurm availability:
1. **Primary mode**: Submits jobs to a Slurm scheduler (in a real HPC environment)
2. **Fallback mode**: Executes jobs locally when Slurm is unavailable (for development/testing)

All functions are designed to operate within the DataSHIELD server-side environment and will automatically detect this context.
...
For computationally intensive jobs, customize Slurm parameters:
...
- Memory/CPU recommendations for Slurm allocation
